//newUser, changePassword, deleteUser
...
async function newUser(src, user, pass) {
    console.log(src);
    console.log(await fs.isAdmin(src));
    //Make sure the user doing this is an admin.
    if (!(await fs.isAdmin(src))) {
        console.log("User trying to perform an admin action isn't an admin.");
        return false;
    }
...
module.exports = (fsArg) => {
    ...
    return {
        ...
        //Change a password.
        changePassword: async (src, user, pass) => {
            //Make sure the user doing this is an admin.
            if ((!(await fs.isAdmin(src))) && (src !== user)) {
                console.log("User trying to perform an admin action isn't an admin.");
                return false;
            }
