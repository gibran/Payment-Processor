...
async function newUser(user, pass, adminArg) {
...
module.exports = async (fsArg) => {
    ...
    return {
        ...
        adminMiddleware: (req, res, next) => {
            //The general account middleware will stop un logged in users from gaining access.
            //This middleware only has to stop logged in users.
            if (typeof(cookies[req.cookies.token]) === "undefined") {
                next();
                return;
            }

            //Make sure the user doing this is an admin.
            if (!(fs.isAdmin(cookies[req.cookies.token].user))) {
                console.log("Ending the request...");
                res.end("<h1>You must be an admin to access this.</h1>");
                return;
            }
